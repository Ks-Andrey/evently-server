...
import { uploadAvatar, validateAvatarDimensions } from '../middlewares/file-upload-middleware';
...
export function createUserRoutes(userController: UserController, tokenManager: ITokenManager): Router {
    ...
    // Защищенные маршруты (должны быть ПЕРЕД общими роутами с параметрами)
    ...
    router.put('/me', auth, validate(editMeSchema), (req, res) => userController.editMe(req, res));
    ...
    router.post('/me/avatar', auth, uploadAvatar, validateAvatarDimensions, (req, res) =>
    ...
    // Специфичные публичные маршруты (должны быть ПЕРЕД общим роутом /:id)
    ...
    // Общие маршруты с параметрами (должны быть в конце)
